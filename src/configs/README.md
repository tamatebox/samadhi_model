...
-   **Modularity**: Configurations are split into logical units (e.g., adapters, vicara, vitakka, decoders).
...
from src.configs.adapters import MlpAdapterConfig
...
1.  Create a new `Enum` member in `enums.py` if it's a new component type.
2.  Define a new `@dataclass` in the relevant component file (e.g., `adapters.py`) inheriting from its `Base*Config`.
...
5.  If it's a top-level parameter or a new nested component, update `SamadhiConfig` in `main.py`.
