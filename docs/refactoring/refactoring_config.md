...
最新のConfigシステムの構造については、[`src/configs/README.md`](src/configs/README.md) を参照してください。
...
```text
# この構成は初期案であり、最終的な実装では src/configs/ 内のファイルがより細かく分割されました。
# 最新のファイル構成は src/configs/README.md を参照してください。
src/configs/
├── __init__.py
...
最新の実装詳細については、各Configファイル（`src/configs/adapters.py`など）および[`docs/config_summary.md`](docs/config_summary.md)を参照してください。

### 3.1 共通基底クラス (`src/configs/base.py`)
全てのConfigの親クラス。`from_dict` による安全な生成と `validate` フックを提供する。

```python
# (中略) 最新のコードは src/configs/base.py を参照
@dataclass
class BaseConfig:
    # ...
...
### 3.2 Enum定義 (`src/configs/enums.py`)
文字列リテラルのタイポを防ぐ。

```python
# (中略) 最新のコードは src/configs/enums.py を参照
...
### 3.3 コンポーネントConfig定義 (`src/configs/components.py`)
**このファイルは削除され、機能は `src/configs/adapters.py`, `src/configs/vicara.py`, `src/configs/vitakka.py`, `src/configs/decoders.py`, `src/configs/objectives.py` に分割されました。**

### 3.4 変換ファクトリ (`src/configs/factory.py`)
Enumを使って分岐し、`from_dict` を呼び出す。非常にシンプルになる。

```python
# (中略) 最新のコードは src/configs/factory.py を参照
# ファクトリ関数は、ObjectiveConfigを含む全てのConfigコンポーネントに対応するようになりました。
# また、型解決のためのより堅牢なロジックが追加されました。
```

### 3.5 全体Config定義 (`src/configs/main.py`)
`BaseConfig` を継承。ネスト構造の復元ロジックも整理。

```python
# (中略) 最新のコードは src/configs/main.py を参照
...
これに伴い、[`src/configs/README.md`](src/configs/README.md)と[`docs/config_summary.md`](docs/config_summary.md)が最新のConfigシステムの構造と使い方を記述しています。
